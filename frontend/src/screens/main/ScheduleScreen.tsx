...
import { View, Text, TouchableOpacity, ActivityIndicator, RefreshControl, ScrollView, Dimensions } from 'react-native';
import { SafeAreaView } from 'react-native-safe-area-context';
import { Clock, User, Coffee, Flag, BookOpen, Heart, Info, CalendarDays, MapPin } from 'lucide-react-native';
...
const ScheduleScreen = () => {
  ...
  const fetchSchedule = async () => {
    try {
      ...
      if (data[todayName] && data[todayName].length > 0) {
        ...
        const firstAvailableDay = dayOrder.find(d => data[d] && data[d].length > 0);
  ...
  const renderTimelineItem = (item: any, index: number, isLast: boolean) => {
    ...
    const config = getTypeConfig(item.tipe || (item.is_non_kbm ? 'istirahat' : 'kbm'), liveStatus);
    ...
    return (
      <View key={index} className="flex-row">
        ...
        <View className="w-[70px] items-end pr-3 pt-4">
          <Text className={`font-bold text-base ${liveStatus ? 'text-blue-600' : 'text-slate-900'}`}>{startTime}</Text>
        ...
        <View className="flex-1 pb-6 relative pl-6">
          ...
          {/* Dot Indikator */}
          <View 
            className={`absolute -left-[4px] top-[18px] w-3 h-3 rounded-full border-2 border-white z-10 shadow-sm ${liveStatus ? 'bg-blue-600 scale-125' : 'bg-slate-300'}`}
          />

          {/* Card */}
          <View 
            className={`rounded-2xl p-4 shadow-sm border ${config.border} ${liveStatus ? 'shadow-blue-200 shadow-lg' : ''}`}
          >
             <View className="flex-row justify-between items-start">
                <View className="flex-1 mr-2">
                   {/* Label Kecil */}
                   <Text className={`text-[10px] font-bold uppercase tracking-wider mb-1 ${liveStatus ? 'text-blue-600' : 'text-slate-400'}`}>
                     {config.label}
                   </Text>
                   ...
                   {!item.is_non_kbm && item.guru && (
                     <View className="flex-row items-center">
                        <User size={14} color={liveStatus ? '#2563eb' : '#94a3b8'} />
                        <Text className={`text-xs font-medium ml-1.5 ${liveStatus ? 'text-blue-700' : 'text-slate-500'}`}>
                          {item.guru}
                        </Text>
                     </View>
                   )}
                ...
                {/* Ikon Kanan Atas */}
                <View className={`w-10 h-10 rounded-full items-center justify-center ${config.bgIcon}`}>
  ...
  return (
    <SafeAreaView className="flex-1 bg-white">
      ...
      <View className="px-6 pt-2 pb-4 border-b border-slate-50 bg-white">
         <View className="flex-row justify-between items-start">
            <View>
              <Text className="text-sm font-semibold text-slate-400 mb-1 uppercase tracking-widest">Jadwal Pelajaran</Text>
          ...
          {availableDays.map((day) => {
            ...
            return (
              <TouchableOpacity
                ...
                className={`mr-3 px-5 py-2.5 rounded-full border ${isActive ? 'bg-slate-900 border-slate-900' : 'bg-slate-50 border-slate-100'}`}
              >
           ...
           {[1,2,3,4,5].map(i => (
              <View key={i} className="flex-row mb-6">
                 ...
                 <View className="flex-1 pl-6 relative">
                    ...
                    {/* Card Body */}
                    <View className="p-4 rounded-2xl border border-slate-100 bg-white shadow-sm flex-row justify-between items-start" style={{ borderLeftWidth: 4, borderLeftColor: '#e2e8f0' }}>
                       <View className="flex-1 mr-4 gap-2">
                          <Skeleton width={60} height={10} borderRadius={4} />
                          <Skeleton width="90%" height={18} />
                          <View className="flex-row items-center mt-1">
                             <Skeleton variant="circle" width={14} height={14} style={{ marginRight: 6 }} />
          ...
          {availableDays.length > 0 && scheduleData[activeDay] ? (
             <View className="pr-6">
                {scheduleData[activeDay].map((item: any, index: number) => 
                   renderTimelineItem(item, index, index === scheduleData[activeDay].length - 1)
                )}
             </View>
          ) : (
